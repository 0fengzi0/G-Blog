		...
		<v-row justify-lg="center">
			...
			<v-col cols="12" lg="7">
				<v-skeleton-loader type="card-avatar" :loading="isLoading" :class="$vuetify.theme.dark ? 'black' : 'white'">
					<v-card elevation="10">
						<v-img class="white--text align-end" height="28vh" :src="$Config.pageCardImgServers + '?tid=' + new Date().getTime() + Math.round(Math.random() * 1000)">
							<v-card-title class="bg-gray-blur">
								<span class="text-truncate d-inline-block">{{ issues.title }}</span>
							</v-card-title>
						</v-img>
						<v-card-title>
							<v-avatar color="indigo" class="mr-3"><v-icon dark>mdi-account-circle</v-icon></v-avatar>
							<div>
			...
			<!-- 文章内容 -->
			<v-col cols="12" lg="7">
				<v-skeleton-loader type="list-item-three-line" :loading="isLoading" :class="$vuetify.theme.dark ? 'black' : 'white'">
					<v-card elevation="10" min-height="200px">
						...
						<v-card-text>
							<v-alert dense border="left" colored-border color="deep-purple">
								...
								<br />
								Issues链接：
								<a :href="'https://github.com/' + $Config.username + '/' + $Config.repo + '/issues/' + issues.number" target="_blank">
